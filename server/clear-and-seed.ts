...
async function clearAndSeedDatabase() {
  ...
  try {
    ...
    await db.execute(sql`TRUNCATE TABLE content_metrics CASCADE`);
    await db.execute(sql`TRUNCATE TABLE ai_generation_tasks CASCADE`);
    await db.execute(sql`TRUNCATE TABLE content CASCADE`);
    await db.execute(sql`TRUNCATE TABLE templates CASCADE`);
    await db.execute(sql`TRUNCATE TABLE niches CASCADE`);
...
export { clearAndSeedDatabase, seedDatabase };
