...
runTest('Database Connection Test', () => {
  try {
    // Check if database connection is working by testing the db.ts file
    const dbPath = path.join(__dirname, 'server', 'db.ts');
    if (!fs.existsSync(dbPath)) {
      throw new Error('Database file not found');
    }
    
    // Test database connection using tsx
    const dbCheck = execSync('npx tsx -e "import { db } from \'./server/db.ts\'; console.log(\'Database connected\')"', { 
      encoding: 'utf8',
      timeout: 10000
    });
    return dbCheck.includes('Database connected') || dbCheck.includes('Database connection successful');
  } catch (error) {
...
// Test 9: Test Scenarios with Seeded Data
